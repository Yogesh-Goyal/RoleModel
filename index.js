    ...
    connection.query(query1, (err, result) => {
         ...
         if(result.length === 0){
           ...
           connection.query(query2,(err,result)=>{
              if(err) throw err;
              if(result.length === 0){
                res.status(400).json({msg: "The user has no access to this page"});
              }
              else{
                const query3 = "select feature from role_feature join user_role on role_feature.roleType = user_role.roleType where user_role.user_id =" + mysql.escape(userId) + "order by role_feature.feature";
